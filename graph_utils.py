...
GraphList = List[Tuple[int, int, float]]  # Each tuple is (Node0, Node1, Distance)

# Load TSP data and convert it into GraphList
def load_tsp_graph_from_file(filepath: str) -> GraphList:
    ...
    # Choose the suitable calculation metric based on the edge_weight_type
    if edge_weight_type == "ATT":
        distance_func = att_distance
    ...
    for i in range(num_nodes):
        for j in range(i + 1, num_nodes):
            distance = distance_func(node_coords[i][1:], node_coords[j][1:])
            graph_list.append((i, j, distance))
...
def att_distance(coord1: Tuple[float, float], coord2: Tuple[float, float]) -> float:
    # ATT distance formula: Rounded to nearest integer
